...
def bbox_rel(image_width, image_height,  *xyxy):
    ...
    w = bbox_w
    h = bbox_h
    return x_c, y_c, w, h, int(x1), int(y1), int(x2), int(y2)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                #clses = []
                #out = []
                out_YOLO = np.array([[1,1,1,1,1,1,1]], dtype=int)
                # Write results
                for *xyxy, conf, cls in det:
                    
                    img_h, img_w, _ = im0.shape  # get image shape
                    
                    x_c, y_c, bbox_w, bbox_h, x1, y1, x2, y2 = bbox_rel(img_w, img_h, *xyxy)
                    obj = [x_c, y_c, x_c + bbox_w, y_c + bbox_h]
                    #bbox_xywh.append(obj)
                    #bbox_xyxy.append(obj)
                    #confs.append([conf.item()])
                    #clses.append([cls.item()])

                    # Yield [bboxkoord x, y, x, y, ID, class, confscore, frame(behövs ej)]
                ...
                out_YOLO = np.delete(out_YOLO, 0, axis=0)
                yield out_YOLO
